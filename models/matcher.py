...
class HungarianMatcher(nn.Module):
    ...
    def get_cost(self, outputs, tgt_ids, tgt_bbox):
        ...
        cost_giou = -generalized_box_iou(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(tgt_bbox))  # same shape
    ...
    def forward_single_stage(self, outputs, targets):
        ...
        tgt_ids = torch.cat([v["labels"] for v in targets])  # total number of inst for the batch (batch_inst, )
        tgt_bbox = torch.cat([v["boxes"] for v in targets])  # (batch_inst, 4)

        C = self.get_cost(outputs, tgt_ids, tgt_bbox)  # [bs, num_query, batch_inst]

        sizes = [len(v["boxes"]) for v in targets]  # list of num_inst of the batch
